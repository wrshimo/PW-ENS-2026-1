document.addEventListener('DOMContentLoaded', () => {
  ...
  /**
   * Processa uma lista de produtos (do banco ou JSON),
   * normaliza os dados e renderiza os cards na tela.
   * @param {Array} products Array de produtos.
   */
  function processAndRenderProducts(products) {
    // transforma preço string -> number, se necessário
    allProducts = products.map((p) => ({
      ...p,
      // O preço do banco já é número, mas o do JSON pode ser string.
  ...
  document.getElementById('generate-fake-btn').addEventListener('click', () => {
    ...
    const fake = generateFakeProducts(n);
  ...
  // 1. Tenta usar os dados `produtosDoBanco` injetados pelo PHP.
  // 2. Se a variável não existir, carrega do `produtos.json` como fallback.
